"use strict";
var Rx = require('rx'), Observable = Rx.Observable, Subject = Rx.Subject, AsyncSubject = Rx.AsyncSubject, ReplaySubject = Rx.ReplaySubject;
...
asyncTest('some like it hot', function () {
    ...
    var result = [];
    setTimeout(function () {
        hotObservable.subscribe(function (x) {
            result.push(x);
        });
    }, 100);
    setTimeout(function () {
...
asyncTest('some like it cold', function () {
    ...
    var result = [];
    setTimeout(function () {
        coldObservable.subscribe(function (x) {
            result.push(x);
        });
    }, 100);
    setTimeout(function () {
...
var collectionAssert = {
    assertEqual: function (actual, expected) {
        ...
        if (expected.length !== actual.length) {
            ok(false, 'Not equal length. Expected: ' +
                expected.length +
                ' Actual: ' +
                actual.length);
            return;
