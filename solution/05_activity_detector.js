...
var createActivityStream_first_solution = function(activity, scheduler) {
  var activityStream = activity.map(function(x) {
    return 1;
  }); //.tap(function (x) { console.log("start activity " + x) });

  var endActivityStream = activity.delay(5000, scheduler).map(function(x) {
    return -1;
...
var redAfter5sec = function(scheduler) {
  return Observable.just('red').delay(5000, scheduler);
};

var createActivityStream = function(activity, scheduler) {
  var stream = activity
    .map(function(x) {
      return Observable.just('green').concat(redAfter5sec(scheduler));
    })
...
test('should get a red signal after 5sec of inactivity', function() {
  ...
  collectionAssert.assertEqual(mockObserver.messages, [onNext(5000, 'red')]);
});

test('should get a green signal after activity', function() {
  var scheduler = new TestScheduler();
  var source = scheduler.createHotObservable(onNext(1000, 'click'));
  var stream = createActivityStream(source, scheduler);
  var mockObserver = scheduler.createObserver();
  stream.subscribe(mockObserver);
  ...
  collectionAssert.assertEqual(mockObserver.messages, [onNext(1000, 'green')]);
});
...
test('should get a red signal subscribing after 6sec of inactivity', function() {
  ...
  collectionAssert.assertEqual(mockObserver.messages, [onNext(6000, 'red')]);
});
