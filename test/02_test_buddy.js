...
test('too soon or too late', function() {
  ...
  // Create hot observable which will start firing
  var xs = scheduler.createHotObservable(onNext(75, 'Hello'), onCompleted(150));
  ...
  collectionAssert.assertEqual(xs.subscriptions, [subscribe(60, 150)]);
});
...
test('moving forward, automatic way', function() {
  ...
  collectionAssert.assertEqual(mockObserver.messages, [__]);
});
...
test('moving forward, manual way', function() {
  ...
  collectionAssert.assertEqual(mockObserver.messages, [onNext(150, 1)]);
